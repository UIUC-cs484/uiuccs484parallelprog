#!/bin/sh

##
#Snippet from
#
#http://stackoverflow.com/questions/59895/can-a-bash-script-tell-what-directory-its-stored-in
#
SCRIPT_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
#
#end snippet
##

BASE_VERSION="v0.5"
TEST_VERSION="v0.5"
CHARM_VERSION="v0.5"
STUD_VERSION="v0.5"

docker build -t cs484_base -t uiuccs484parallelprog/cs484_base:latest -t uiuccs484parallelprog/cs484_base:${BASE_VERSION}  ${SCRIPT_DIR}/base
docker build -t cs484_test -t uiuccs484parallelprog/cs484_test:latest -t uiuccs484parallelprog/cs484_test:${TEST_VERSION} ${SCRIPT_DIR}/wgtest
docker build -t cs484_charm -t uiuccs484parallelprog/cs484_charm:latest -t uiuccs484parallelprog/cs484_charm:${CHARM_VERSION} ${SCRIPT_DIR}/wcharm
docker build -t cs484_student -t uiuccs484parallelprog/cs484_student:latest -t uiuccs484parallelprog/cs484_student:${STUD_VERSION} ${SCRIPT_DIR}/student

docker push uiuccs484parallelprog/cs484_base:${BASE_VERISON}
docker push uiuccs484parallelprog/cs484_test:${TEST_VERSION}
docker push uiuccs484parallelprog/cs484_charm:${CHARM_VERSION}
docker push uiuccs484parallelprog/cs484_student:${STUD_VERSION}

docker push uiuccs484parallelprog/cs484_base:latest
docker push uiuccs484parallelprog/cs484_test:latest
docker push uiuccs484parallelprog/cs484_charm:latest
docker push uiuccs484parallelprog/cs484_student:latest

